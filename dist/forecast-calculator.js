 ...
 */
function toDecimalRate(inputRate) {
 ...
 */
function getOrDefaultRate(assetValue, defaultPercentage) {
    const raw = assetValue != null ? assetValue : defaultPercentage;
    return toDecimalRate(raw);
}
function getOrDefaultNumber(assetValue, defaultVal) {
 ...
 */
function computeAnnualPiPayment(principal, annualInterest, yearsOfRepayment) {
    ...
    // (1 + i)^(-n)
    const denom = Big(1).minus(Big(1).plus(i).pow(-n) // Use negative number directly instead of Big.times(-1)
    );
    if (denom.eq(0)) {
        // avoid division by zero if something is off
        return Big(0);
    }
    return principal.times(i.div(denom));
}
/**
 ...
 */
export function calculateForecast(profile, settings) {
    ...
    const assetsProcessed = profile.assets.map((asset) => {
        ...
        const capitalGrowthRate = getOrDefaultRate(asset.capitalGrowthRate, settings.defaultCapitalGrowthRate);
        const incomeGrowthRate = getOrDefaultRate(asset.incomeGrowthRate, settings.defaultIncomeGrowthRate);
        const expenseGrowthRate = getOrDefaultRate(asset.expenseGrowthRate, settings.defaultExpenseGrowthRate);
        const loanInterestRate = getOrDefaultRate(asset.loanInterestRate, settings.defaultLoanInterestRate);
        ...
        return {
            ...
            incomePerYear: getOrDefaultNumber(asset.incomePerYear, 0),
            incomeGrowthRate,
            ...
            loanToValueRatio: getOrDefaultRate(asset.loanToValueRatio, settings.defaultLoanToValueRatio),
            loanInterestRate,
            loanInterestOnlyPeriod: getOrDefaultNumber(asset.loanInterestOnlyPeriod, settings.defaultLoanInterestOnlyPeriod),
    ...
    const assetStates = assetsProcessed.map((asset) => {
        ...
        const annualPiPayment = computeAnnualPiPayment(originalPrincipal, interestRate, finalPiYears);
        return {
    ...
    for (let yearOffset = 0; yearOffset < 50; yearOffset++) {
        ...
        assetsProcessed.forEach((asset, i) => {
            ...
            const currentValue = Big(asset.purchaseMarketValue).times(Big(1).plus(asset.capitalGrowthRate).pow(Math.max(0, yearsHeld)));
            // 4.2 Income (annual)
            // Just use the annual income directly instead of calculating from weekly
            const baseAnnualIncome = Big(asset.incomePerYear);
            const grownAnnualIncome = baseAnnualIncome.times(Big(1).plus(asset.incomeGrowthRate).pow(Math.max(0, yearsHeld)));
            ...
            const grownExpenses = baseExpenses.times(Big(1).plus(asset.expenseGrowthRate).pow(Math.max(0, yearsHeld)));
