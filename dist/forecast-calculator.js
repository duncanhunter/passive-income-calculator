...
function computeAnnualPiPayment(principal, annualInterest, yearsOfRepayment) {
  if (yearsOfRepayment <= 0) {
    return big_default(0);
  }
  if (annualInterest.eq(0)) {
    return principal.div(yearsOfRepayment);
  }
  const i = annualInterest;
  const n = yearsOfRepayment;
  const denom = big_default(1).minus(
    big_default(1).plus(i).pow(-n)
    // Use negative number directly instead of Big.times(-1)
  );
  if (denom.eq(0)) {
    return big_default(0);
  }
...
function calculateForecast(profile, settings) {
  ...
  const assetsProcessed = profile.assets.map((asset) => {
    ...
    if (loanTermYears == null) {
      loanTermYears = asset.type === "principalPlaceOfResidence" ? DEFAULT_PPOR_LOAN_TERM_YEARS : DEFAULT_LOAN_TERM_YEARS;
    }
    return {
      ...asset,
      ...
      loanTermYears
      // Store the loan term we determined
    };
  });
  const assetStates = assetsProcessed.map((asset) => {
    const originalPrincipal = big_default(asset.loanAmount);
    const interestRate = big_default(asset.loanInterestRate);
    ...
    return {
      currentLoanBalance: originalPrincipal,
      // start at original principal
      annualPiPayment
  ...
  for (let yearOffset = 0; yearOffset < 50; yearOffset++) {
    ...
    let totalLoanBalance = big_default(0);
    ...
    assetsProcessed.forEach((asset, i) => {
      ...
      const baseExpenses = big_default(asset.expensesPerYear);
      const grownExpenses = baseExpenses.times(
        big_default(1).plus(asset.expenseGrowthRate).pow(Math.max(0, yearsHeld))
      );
      let interestPaid = big_default(0);
      let principalPaid = big_default(0);
      const loanBalanceBefore = assetStates[i].currentLoanBalance;
      let loanBalanceAfter = loanBalanceBefore;
      if (loanBalanceBefore.gt(0)) {
        const interestRate = big_default(asset.loanInterestRate);
        const ioPeriod = asset.loanInterestOnlyPeriod;
        const loanTermYears = asset.loanTermYears;
        const yearOfLoan = yearsHeld + 1;
        const piYears = loanTermYears - ioPeriod;
        if (yearOfLoan > loanTermYears) {
          interestPaid = big_default(0);
          principalPaid = big_default(0);
          loanBalanceAfter = big_default(0);
        } else {
    ...
    let totalPrincipalPaid = big_default(0);
    ...
    const gapToIncomeGoal = big_default(profile.passiveIncomeGoal).minus(netIncome);
    const realGapToGoal = big_default(profile.passiveIncomeGoal).minus(cashFlowAfterAllPrincipal);
